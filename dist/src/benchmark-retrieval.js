import { MAX_BENCHMARK_ABS_SCORE, MAX_BENCHMARK_DIAGNOSTICS, MAX_BENCHMARK_TEXT, benchmarkRecordKey, } from "./benchmarks.js";
 ...
 */
export const ALLOWED_ARTIFICIAL_ANALYSIS_METRICS = {
...
function bounded(value, max = MAX_BENCHMARK_TEXT) {
    if (typeof value !== "string")
        return undefined;
    const text = value.trim();
    if (text.length === 0 || text.length > max || /[\u0000-\u001f\u007f]/u.test(text))
        return undefined;
    return text;
}
 ...
 */
export function parseSourceModelMap(value, expectedSource = ARTIFICIAL_ANALYSIS_SOURCE) {
    if (!record(value))
        throw new Error("benchmark mapping must be a JSON object");
    const source = bounded(value.source, 64);
    if (source !== expectedSource)
        throw new Error(`benchmark mapping source must be "${expectedSource}"`);
    if (!Array.isArray(value.mappings))
        throw new Error("benchmark mapping must contain a mappings array");
    if (value.mappings.length === 0)
        throw new Error("benchmark mapping must contain at least one entry");
    if (value.mappings.length > MAX_SOURCE_MAP_ENTRIES)
        throw new Error(`benchmark mapping must contain at most ${MAX_SOURCE_MAP_ENTRIES} entries`);
    ...
    value.mappings.forEach((entry, index) => {
        ...
        if (!sourceId)
            throw new Error(`mappings[${index}].sourceId must be a bounded non-empty string`);
        if (mappings.has(sourceId))
            throw new Error(`duplicate sourceId in benchmark mapping: ${sourceId}`);
        const identity = identityFrom(entry);
        if (!identity)
            throw new Error(`mappings[${index}] must name exact provider and id`);
        mappings.set(sourceId, identity);
    });
    return { source, mappings };
}
function responseDate(now) {
    return now.toISOString().slice(0, 10);
}
/** The response-level index version, bounded exactly like a persisted version. */
function indexVersionLabel(value) {
...
export function isAllowedRetrievalEndpoint(url) {
    try {
 ...
 */
function withDeadline(value, timeoutMs, onTimeout) {
...
async function readBoundedBody(body, maxBodyBytes) {
    if (!body)
    ...
    try {
        ...
        catch { /* cancellation may already release it */ }
    }
...
async function fetchRetrievedPage(fetcher, url, credential, timeoutMs, maxBodyBytes) {
    if (!isAllowedRetrievalEndpoint(url))
 ...
 */
function normalizeRetrievedModel(datum, ctx) {
    if (!record(datum))
        return { records: [], diagnostics: [{ category: "malformed" }] };
    const sourceId = bounded(datum.id, 200);
    if (!sourceId)
        return { records: [], diagnostics: [{ category: "malformed" }] };
    const identity = ctx.mapping.get(sourceId);
    if (!identity)
        return { records: [], diagnostics: [{ category: "unmatched-model", sourceId }] };
    const model = modelKey(identity);
    if (!ctx.known.has(model))
        return { records: [], diagnostics: [{ category: "unmatched-model", sourceId, model }] };
    if (!record(datum.evaluations))
        return { records: [], diagnostics: [{ category: "malformed", sourceId, model }] };
    const records = [];
    const diagnostics = [];
    let usable = 0;
    let unsupported = 0;
 ...
 */
export async function retrieveArtificialAnalysis(options) {
    const records = [];
    const diagnostics = [];
    const credential = typeof options.credential === "string" ? options.credential.trim() : "";
    if (!credential)
        return { records, diagnostics: [sourceUnavailable()] };
    ...
    for (let page = 1; page <= maxPages && !capped; page += 1) {
        ...
        const ctx = { mapping: options.mapping, known, version: indexVersionLabel(body.intelligence_index_version), date };

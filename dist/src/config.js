...
import { DEFAULT_HEALTH } from "./health.js";
 ...
 * than ignored.
 */
function readHealthConfig(value) {
    if (value === undefined)
        return undefined;
    if (!isRecord(value))
    ...
    if (value.enabled !== undefined) {
        if (typeof value.enabled !== "boolean")
            throw new Error("health.enabled must be a boolean");
...
function readCandidate(value, index, now) {
    if (!isRecord(value))
...
export function parseConfig(raw, now = Date.now()) {
...
export function agentDirPath(env = process.env) {
...
export function resolveConfig(input) {
    ...
    if (fs.existsSync(projectPath))
        return { config: readConfig(projectPath), source: "project", path: projectPath };
    const globalPath = input.agentDir
        ? path.join(input.agentDir, "delegateau.json")
        : globalConfigPath(env);
    if (fs.existsSync(globalPath))
        return { config: readConfig(globalPath), source: "global", path: globalPath };

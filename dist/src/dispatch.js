...
import { classifyChildHealth } from "./health.js";
...
import { buildRouteTrace, mergeExcludedCandidates } from "./route-trace.js";
...
function update(options, dispatchId, status, text, details = {}) {
...
export async function executeDispatch(options) {
    ...
    let healthCategory;
    const traceFor = (outcome) => buildRouteTrace({
        dispatchId,
     ...
     */
    const recordChildHealth = (result) => {
        ...
        if (verdict.kind !== "success")
            healthCategory = verdict.category;
    ...
    try {
        ...
        if (controller.signal.aborted)
            throw new AdmissionError("Delegation cancelled before launch", "cancelled");
        ...
        if (result.status !== "success")
            throw new DispatchFailure({ text, details });
        ...
        if (error instanceof DispatchFailure)
            throw error;
        if (cancelled)
            throw new DispatchFailure({ text: `Dispatch ${dispatchId}: cancelled`, details: { dispatchId, status: "cancelled" } });
...
export function validateAssignment(config, assignment) {
    if (assignment.task.trim() === "")
        throw new DispatchFailure({ text: "task must not be empty", details: { status: "launch-error", error: "empty-task" } });
    if (assignment.task.length > config.limits.maxTaskChars)
        throw new DispatchFailure({ text: `task exceeds ${config.limits.maxTaskChars} characters`, details: { status: "launch-error", error: "task-too-large" } });
    if (assignment.context && assignment.context.length > config.limits.maxContextChars)
        throw new DispatchFailure({ text: `context exceeds ${config.limits.maxContextChars} characters`, details: { status: "launch-error", error: "context-too-large" } });
    if (assignment.expectedOutput && assignment.expectedOutput.length > config.limits.maxExpectedOutputChars)
        throw new DispatchFailure({ text: `expectedOutput exceeds ${config.limits.maxExpectedOutputChars} characters`, details: { status: "launch-error", error: "expected-output-too-large" } });
}
export function validateConfiguredChildTools(agent) {
    const invalid = agent.tools.filter((tool) => tool === "delegate_task" || (!CHILD_TOOLS.has(tool) && (agent.childExtensions?.length ?? 0) === 0));
    if (invalid.length > 0)
        throw new DispatchFailure({ text: `Child tools are not approved: ${invalid.join(", ")}`, details: { status: "launch-error", error: "invalid-child-tools" } });
}

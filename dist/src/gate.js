...
export class DelegationGate {
    ...
    async ensure(input, runtime) {
        ...
        if (this.active && sameRequest)
            return this.active.decision;
        if (this.active)
            this.invalidate("replaced by a new request");
        ...
        const finish = (result) => {
            if (!this.active || this.active.decision.decisionId !== decisionId)
                return this.lastDecision ?? result;
        ...
        try {
            if (!cancelled) {
                cancelled = new Promise((_, reject) => {
            ...
            if (!this.active || this.active.decision.decisionId !== decisionId || controller.signal.aborted)
                throw abortError();
            if (answer.recommendation !== "local" && answer.recommendation !== "delegate")
                throw new Error("Jev returned an invalid delegation recommendation");
            if (answer.recommendation === "delegate" && !delegateAllowed)
                throw new Error("Jev recommended delegation but no usable child is available");
            return finish({
                ...pending,
                status: "applied",
                source: "jev",
            ...
            if (this.active?.decision.decisionId === decisionId && this.active.decision.source === "user-override")
                return this.active.decision;
    ...
    markExecution(kind) {
        ...
        const execution = current === "none" || current === kind ? kind : "mixed";
        this.active.decision = { ...this.active.decision, execution };
        this.lastDecision = this.active.decision;
    }
    override(kind) {
        if (!this.active)
            throw new Error("There is no active delegation decision to override");
        const input = this.active.input;
        if (kind === "delegate" && !baseAllowsDelegate(input))
            throw new Error("Delegation override is unavailable because no eligible child can be launched");
        if (kind === "local" && !baseAllowsLocal(input))
            throw new Error("Local override would widen the active base mode");
...
export class JevDelegationSelector {
    client;
    timeoutMs;
    /**

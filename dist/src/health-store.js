...
import { abandonTrial, beginTrial, emptyHealthState, getCircuitState, isHealthCategory, recordHealthFailure, recordHealthSuccess, resolveHealthConfig, } from "./health.js";
export function healthPath(agentDir = getAgentDir()) {
...
function normalizeRecord(raw) {
    ...
    const failures = Array.isArray(record.failures)
        ? record.failures.filter((ts) => typeof ts === "number" && Number.isFinite(ts))
        : [];
...
export function loadHealthState(filePath) {
    try {
...
export function saveHealthState(filePath, state) {
...
export class HealthStore {
    ...
    expiredTrialCount(now) {
        ...
        for (const [key, record] of Object.entries(this.state.models)) {
            if (record.trialActive !== true)
                continue;
            if (getCircuitState(this.state, key, at, this.config).trialActive)
                continue;
            count += 1;
        }
        return count;
    ...
    recordFailure(key, category, now) {
        if (!resolveHealthConfig(this.config).enabled)
            return;
    ...
    resolvePath(configured, cwd = this.cwd) {
        if (!configured)
            return healthPath();

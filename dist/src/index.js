...
function configuredAgent(config, name) {
    ...
    if (!agent)
        throw new Error(`Unknown trusted agent "${name}"; configure it in ${CONFIG_FILE_NAME}`);
...
function validateAssignment(config, params) {
    ...
    if (params.task.length > config.limits.maxTaskChars)
        throw new Error(`task exceeds ${config.limits.maxTaskChars} characters`);
...
function validateChildTools(agent) {
    ...
    if (invalid.length > 0)
        throw new Error(`Child tools are not approved: ${invalid.join(", ")}`);

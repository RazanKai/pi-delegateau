...
function configuredAgent(config, name) {
    const agent = config.agents[name];
    if (!agent)
        throw new DelegationToolError({ text: `Unknown trusted agent "${name}"; configure it in ${CONFIG_FILE_NAME}`, details: { status: "launch-error", error: `unknown-agent:${name}` } });
...
function resolveLaunchModelImpl(request, jevChoose) {
    const pin = request.agent.model;
    if (pin)
        return request.candidates.some((candidate) => modelKey(candidate.identity) === modelKey(pin)) ? pin : undefined;
...
function buildGateSelector(config) {
    ...
    if (config.delegationDecision === "manual")
        return undefined;
    if (!config.allowExternalSensing)
        return undefined;
    try {
...
export default function (pi) {
    ...
    try {
        ...
        const foreign = existing.filter((tool) => tool.name === TOOL_NAME && tool.sourceInfo?.source !== undefined);
        if (foreign.length > 0) {
            const owner = foreign[0].sourceInfo?.path ?? foreign[0].sourceInfo?.source ?? "another extension";
        ...
        if (error instanceof Error && error.message.includes("double-register"))
            throw error;
    ...
    pi.registerCommand("delegateau", {
        ...
        handler: async (args, ctx) => {
            ...
            if (command === "override" || ["local", "delegate", "manual"].includes(command)) {
                const requested = (command === "override" ? words[1] : command);
            ...
            const requested = command === "enable" ? words[1] ?? config.mode : command;
            ...
            const result = mode.activate(requested);
    ...
    pi.on("tool_call", async (event) => {
        ...
        if (event.toolName === TOOL_NAME)
            shadowedDelegateTaskCalls = true;
        ...
        if (!result && event.toolName !== TOOL_NAME)
            gate.markExecution("local");
        return result;
    });
    pi.on("before_agent_start", async (event, ctx) => {
        ...
        if (current !== "normal")
            additions.push(`# DELEGAU MODE (active)\n${policyForMode(current).prompt}`);
    ...
    pi.on("input", async (event, ctx) => {
        ...
        if (!invalidated)
            return;
        if (invalidated.policy === "jev-enforce")
            mode.setRequestRestriction("blocked");
        try {
            const config = loadConfig(ctx.cwd);

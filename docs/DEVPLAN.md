...
## M10 — Explicit onboarding and live-pool bootstrap

**Goal:** let a fresh project review a usable delegateau pool without hand-authoring models or silently spending quota.

Work:

1. Keep extension import/session start and ordinary setup review side-effect free. Add explicit benchmark subcommands: `/delegateau setup import <file>` performs no network access; `/delegateau setup retrieve artificial-analysis <mapping-file>` runs only on direct user invocation.
2. Define one bounded normalized record with exact Pi model identity, source identifier/HTTPS URL, benchmark, version, metric, date plus explicit date provenance (`dateKind`: source-reported measurement versus retrieval snapshot), provenance, finite score, unit and direction. Validate length/count/date/URL bounds, deduplicate exact records on the full observation identity (including date and score) while keeping a separate comparison key for dominance/pruning, preserve multiple unlike metrics/versions, and emit only sanitized ignored diagnostics.
3. Persist validated acquisition results in the Pi agent state area, outside project policy config. Match only exact identities currently present in Pi's live authenticated registry, and revalidate prior cache entries against the current live registry and clock before merging so stale/unmatched entries cannot crowd out fresh evidence under the per-model cap. Unknown, stale, future, malformed, unavailable and unmatched input remains explicitly ignored/unmeasured.
4. Implement a validated local JSON import adapter and an explicit Artificial Analysis v2 retrieval adapter. The retrieval adapter calls the documented Free-tier `GET https://artificialanalysis.ai/api/v2/language/models/free` with `ARTIFICIAL_ANALYSIS_API_KEY` from the process environment and requires a user-supplied mapping from stable Artificial Analysis model `id`s to exact live Pi identities; it never guesses aliases from names or slugs. The credentialed network call is bounded by a deadline covering the request and body read and by a hard response-byte cap enforced before JSON parsing; only the documented Free-tier composite indices are promoted and other numeric fields become bounded unsupported diagnostics. The credentialed network call is not issued during import review, session start, ordinary setup, or dispatch. Sources/providers without a reliable API remain named as unmeasured and may use the import adapter.
5. Preserve records through `setupConfig`, config parsing, `CandidateProfile`, and runtime enrichment. Dominance may compare only identical source/benchmark/version/metric/unit/direction records and must not penalize an unmeasured candidate.
6. Add bounded concise benchmark facts to every measured candidate's Choice criterion and structured Jev candidate state. Extend sanitized route traces/receipts with the exact bounded records offered to Jev and an explicit offered flag; never retain raw import/API/task/source bodies, credentials, or URL query/fragment.
7. Update setup review/status/help and README with measured/unmeasured counts, explicit commands, supported source adapters, and limits. Keep benchmark quality subordinate to eligibility, quota, reachability, runtime health, cost and task fit.
8. Verify strict red/green vertical slices for import/config, setup projection/pruning, Jev state/criteria, route evidence, explicit retrieval, and the registered dispatch path. Then run the canonical hermetic suite, ordinary all-extension discovery, explicit current-source import/retrieval, and a credentialed Jev-to-child dispatch proving structured records appear in sanitized route evidence.

**Acceptance target:** T22.
...
| T22 | R15 | M10 | DONE | Bounded normalized records (`src/benchmarks.ts`: `normalizeBenchmarkDocument`, `collectBenchmarkEvidence`, `mergeBenchmarkReport`, `revalidateBenchmarkCache`) persist in the Pi agent-state cache and cross `setupConfig`/config/`CandidateProfile` into Jev criteria/state (`src/jev.ts`) and sanitized route evidence with an explicit `offeredToJev` flag (`src/dispatch.ts`). Explicit commands `/delegateau setup import <file>` (no network) and `/delegateau setup retrieve artificial-analysis <mapping>` implement the local JSON adapter and the Artificial Analysis v2 Free-tier adapter (`src/benchmark-retrieval.ts`, `GET https://artificialanalysis.ai/api/v2/language/models/free`). Tests: `tests/benchmarks.test.ts`, `tests/benchmark-retrieval.test.ts`, `tests/onboarding.test.ts`, `tests/jev.test.ts`, `tests/route-trace.test.ts`, `tests/extension-entry-benchmark.test.ts` (real TypeSafe SDK transport; `offered:true` only when the chooser ran), `tests/extension-setup-benchmark.test.ts`; review hardening covers exact observation dedup (date+score) with a separate dominance comparison key, prior-cache revalidation before the per-model cap, a documented Free-tier composite-index allowlist (other numerics are bounded `unsupported-metric` diagnostics), explicit `dateKind` date provenance, and deadline + pre-parse byte bounds on each retrieval round trip (hung transport/body, oversize). A config-embedded record that has merely AGED OUT is demoted to a bounded diagnostic with its candidate left in the pool and unmeasured, while a shape fault still refuses the config, so a valid config cannot become unloadable when a record expires. A later fix round added: a credential-scoped redirect refusal and single-host allow-list, acquisition-time normalization so the adapter cannot emit a record its own parser rejects, newest-first merge so the per-model cap evicts the oldest evidence instead of the record just acquired (with bounded eviction diagnostics and per-command retained counts), an explicit `offeredCount`/`truncated` pair on the route trace with the trace cap raised 64->512, the 730-day age rule applied to config-embedded records too, and trial-claim expiry so a killed Pi cannot strand a half-open candidate. Mapping is derived, not hand-typed: `scripts/build-aa-mapping.py` reads `pi --list-models` and the live AA Free-tier catalog and emits only exact-slug matches (`/delegateau setup retrieve artificial-analysis <mapping-file>`). Canonical `env -u TYPESAFE_API_KEY -u TYPESAFE_BASE_URL npm run check` passes **27 files / 241 tests**; every fix has a regression that fails on the pre-fix tree and passes after it. Live: a fresh Pi 0.87.1 headless session loading the updated source ran a Jev-selected child and the receipt recorded the exact synthetic record with `offeredToJev=true` (`/home/nazar/.hermes/cache/scratch/t22-live-20260924T140403Z/`); a credentialed AA retrieval then ran end to end (51 records across 19 live identities, 100 bounded diagnostics, stored records re-validated and round-tripping with no loss) and a real Jev selection received per-candidate benchmark facts. Evidence upkeep is self-deciding: `scripts/benchmark-evidence.mjs` compares evidence age and the current live `pi --list-models` against its recorded baseline and refreshes only when warranted (age, an expired record, records that stopped validating, or a changed model list), projecting records onto the existing candidates and re-parsing the result to prove the config still loads. The extension itself still never fetches implicitly. Remaining: 6 of 25 live Pi models have no exact AA slug match and stay visibly unmeasured rather than being aliased; no provider-independent record was imported live. |

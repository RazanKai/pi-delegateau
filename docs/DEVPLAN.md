...
Work: add an explicit setup command and focused onboarding module; read Pi `getAvailable()` plus `hasConfiguredAuth()`, preserve exact registry metadata, safely project only policy into config, and require confirmation before writes. Keep probes sequential/cached and quota measurement opt-in; implement deterministic evidence/pruning and role-template contracts. Provider-specific benchmark fetchers and automatic measurement adapters remain deferred: unavailable evidence is labelled unmeasured, never fabricated.
...
| T22 | R15 | M10 | PARTIAL | `src/onboarding.ts` and `tests/onboarding.test.ts` provide deterministic discovery, evidence, pruning, templates and confirmation contracts. Explicit command/dry-run and provider-specific probe/benchmark adapters require live verification. |

...
Reachability probes use the child launch shape, run sequentially, cache classified failures, and are explicit setup stages; missing evidence never removes a candidate. Provider-owned telemetry and credential shape may label access mode, while user overrides remain authoritative. Quota-cost measurement is bounded and opt-in; no measurement occurs against a near-exhausted bucket. Benchmark acquisition is an explicit setup stage. A local structured import (`/delegateau setup import <file>`) performs no network access; a named source adapter may fetch only when the user explicitly invokes that adapter. Import, extension load, session start, ordinary setup review, and dispatch never fetch benchmark data. The first supported network adapter is Artificial Analysis's authenticated v2 model-data API (`/delegateau setup retrieve artificial-analysis <mapping>`), which calls the documented Free-tier language-models endpoint `GET https://artificialanalysis.ai/api/v2/language/models/free` with the credential from the process environment (`ARTIFICIAL_ANALYSIS_API_KEY`) and requires an explicit mapping from each stable Artificial Analysis model `id` to an exact live Pi `provider/model` identity; it never infers an alias from a model name or a loose slug similarity. The mapping is a reviewable file that may be written by hand or produced by `scripts/build-aa-mapping.py`, which reads the live `pi --list-models` registry and the live AA catalog and emits a pair only when exactly one AA entry matches a live Pi model after documented canonicalization (case and punctuation stripped); a Pi model with no match, or with more than one, is omitted rather than aliased, so an unmatched model stays visibly unmeasured. Sources without a stable compatible API use the validated local import adapter and remain unmeasured when no trustworthy record is supplied. The legacy `/api/v2/data/llms/models` path retires 2026-11-04 and is not used. Every explicit retrieval round trip is bounded by a deadline covering the request and body read and by a hard response-byte cap enforced before JSON parsing (never only after materializing the parsed object); a hung transport, a hung body, or an oversize body becomes a sanitized diagnostic and writes no cache. The request is issued only to that one documented HTTPS host and follows no redirect, so the credential cannot be replayed to another origin. Acquisition accepts only what the record parser accepts: an over-long or control-character-bearing version label is replaced by the same placeholder as a missing one, and an out-of-range score is reported as a bounded diagnostic instead of becoming a record, so an acquisition can never report success and then disappear at the next read. The adapter promotes only the Free-tier composite indices whose meaning the contract documents (Artificial Analysis Intelligence Index, Coding Index, Agentic Index), carries the response-level `intelligence_index_version` only for the Intelligence Index, treats the coding and agentic indices as documented-not-separately-versioned, and classifies any other numeric `evaluations` field as an unsupported diagnostic rather than inventing a metric, unit, direction, or version.

Normalized benchmark records are bounded and preserve exact model identity, source identifier and HTTPS URL, benchmark, version, metric, observation date with an explicit `dateKind` (`source-reported` measurement date versus `retrieval-snapshot` adapter fetch date), provider/independent provenance, finite score, unit, and score direction. Multiple records per model are retained; exact record identity includes date and score, so genuinely distinct observations are preserved while the separate comparison key (source/benchmark/version/metric/unit/direction/provenance) remains the only basis for dominance. Unlike metrics and incompatible benchmark versions are never collapsed. Validated acquisition results persist in a bounded cache under the Pi agent state area (`<agentDir>/delegateau/benchmarks.json`), outside project policy configuration, and are revalidated against the current live authenticated registry when setup reads them; a record can only attach to the exact `provider/model` identity it names. Merging an acquisition report revalidates prior cache entries against the current live registry and clock before the per-model cap is applied, so stale, future-dated, or unmatched entries cannot crowd out fresh evidence: freshly acquired records are ordered first, so the cap evicts the oldest prior evidence rather than the record just acquired, and each eviction is reported as a bounded diagnostic. Identical diagnostics are collapsed rather than accumulated on every merge. The documented 730-day age rule applies to every entry point, including records written directly into a configuration. A config-embedded record that is malformed, names a different model, or duplicates another is an authoring error and refuses the config; a record that merely AGED OUT or is future-dated is demoted — dropped with a bounded diagnostic while its candidate stays in the pool, unmeasured — because age is not an authoring mistake and a config that is valid when written must not become unloadable when a record expires. The drop is reported on the status/review surface, and nothing refreshes evidence implicitly: retrieval remains an explicit command, so acquired evidence ages visibly rather than being silently renewed. Imported records that are malformed, stale, future-dated, unmatched to the current live registry, untrusted (provenance other than provider/independent), or unavailable are ignored with bounded sanitized diagnostics and leave that candidate visibly unmeasured. Diagnostics carry only a category and bounded identity, never raw source bodies or unsafe URLs. Pruning removes confirmed-unreachable models and only models proven dominated across all of their comparable records; missing evidence never ranks an unmeasured model below a measured one.

Validated records cross the setup cache/config boundary as structured candidate data. The model chooser receives a bounded structured record list and concise per-candidate benchmark facts with source/version/date/provenance visible. A bounded sanitized route trace records the exact records offered to Jev and whether they were offered, the count actually offered, and whether its own storage cap clipped the list, so a reader cannot mistake a partial list for the whole one; it is written through the existing receipt path and contains no imported raw body, task/source body, credentials, or URL query/fragment. The offered flag is true only when the model chooser was actually invoked for that dispatch; a fixed/pinned/single-candidate dispatch records the exact available records with `offered: false`. Benchmark quality remains subordinate to eligibility, reachability, health, quota, budget, and task fit.
...
| T22 | Explicit setup discovers configured live registry identities, imports or explicitly retrieves bounded normalized benchmark records, preserves them through config/runtime into Jev criteria/state and sanitized route evidence, keeps unmatched/stale/incompatible data visibly ignored or unmeasured, and writes only after confirmation; probes/measurement/retrieval remain opt-in | R15 |
...
- LiveBench official leaderboard/data: https://livebench.github.io/ and https://github.com/LiveBench/LiveBench
- SWE-bench official leaderboard data: https://github.com/swe-bench/swe-bench.github.io/blob/master/data/leaderboards.json

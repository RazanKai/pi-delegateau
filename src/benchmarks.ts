...
function boundedText(value: unknown): string | undefined {
  ...
  if (text.length === 0 || text.length > MAX_BENCHMARK_TEXT || /[\u0000-\u001f\u007f]/u.test(text)) return undefined;
  return text;
}

function safeSourceUrl(value: unknown): string | undefined {
  if (typeof value !== "string" || value.length > MAX_BENCHMARK_URL) return undefined;
  try {
...
function diagnostic(category: BenchmarkDiagnosticCategory, index?: number, identity?: ModelIdentity): BenchmarkDiagnostic {
  return {
    category,
...
function parseIdentity(value: unknown): ModelIdentity | undefined {
...
function parseShape(value: unknown): { evidence?: BenchmarkEvidence; category?: BenchmarkDiagnosticCategory; identity?: ModelIdentity } {
  if (!record(value)) return { category: "malformed" };
  ...
  const direction: BenchmarkDirection | undefined = value.direction === "higher-is-better" || value.direction === "lower-is-better" ? value.direction : undefined;
  if (typeof value.sourceUrl === "string" && !sourceUrl) return { category: "unsafe-source-url", ...(identity ? { identity } : {}) };
...
export function benchmarkComparisonKey(value: BenchmarkEvidence): string {
 ...
 */
export function benchmarkRecordKey(value: BenchmarkEvidence): string {
...
export function normalizeBenchmarkDocument(value: unknown, options: NormalizeBenchmarkOptions): BenchmarkImportReport {
  ...
  if (value.records.length > input.length) diagnostics.push(diagnostic("limit-exceeded"));
...
export function benchmarkCachePath(agentDir = getAgentDir()): string {
...
export function collectBenchmarkEvidence(candidates: readonly CandidateProfile[], limit = MAX_OFFERED_BENCHMARKS): BenchmarkEvidence[] {
  const collected: BenchmarkEvidence[] = [];
  const seen = new Set<string>();
...
export function readBenchmarkCache(filePath = benchmarkCachePath()): BenchmarkCache | undefined {
  try {
    const value = JSON.parse(fs.readFileSync(filePath, "utf8")) as BenchmarkCache;
...
export function writeBenchmarkCache(cache: BenchmarkCache, filePath = benchmarkCachePath()): void {
 ...
 */
function mergeRecords(newest: readonly BenchmarkEvidence[], older: readonly BenchmarkEvidence[]): { records: BenchmarkEvidence[]; diagnostics: BenchmarkDiagnostic[] } {
  const merged = new Map<string, BenchmarkEvidence>();
  for (const entry of newest) merged.set(benchmarkRecordKey(entry), entry);
 ...
 */
export function mergeBenchmarkReport(prior: BenchmarkCache | undefined, report: BenchmarkImportReport, options: NormalizeBenchmarkOptions): BenchmarkCache {
...
function dedupeDiagnostics(diagnostics: readonly BenchmarkDiagnostic[]): BenchmarkDiagnostic[] {
  const seen = new Set<string>();
  const unique: BenchmarkDiagnostic[] = [];
 ...
 */
export function revalidateBenchmarkCache(cache: BenchmarkCache | undefined, options: NormalizeBenchmarkOptions): BenchmarkImportReport {
  return normalizeBenchmarkDocument({ version: BENCHMARK_CACHE_VERSION, records: cache?.records ?? [] }, options);
}

export async function importBenchmarkFile(filePath: string, options: NormalizeBenchmarkOptions & {
  readFile?: (filePath: string) => Promise<string>;
  readCache?: () => Promise<BenchmarkCache | undefined>;
  writeCache?: (cache: BenchmarkCache) => Promise<void>;
}): Promise<{ imported: number; retained: number; dropped: number; records: BenchmarkEvidence[]; diagnostics: BenchmarkDiagnostic[] }> {
  const readFile = options.readFile ?? (async (target: string) => fs.promises.readFile(target, "utf8"));
  const text = await readFile(filePath);
  if (Buffer.byteLength(text, "utf8") > MAX_BENCHMARK_IMPORT_BYTES) throw new Error(`Benchmark import exceeds ${MAX_BENCHMARK_IMPORT_BYTES} bytes`);
  let raw: unknown;
  ...
  if (options.writeCache) await options.writeCache(cache); else writeBenchmarkCache(cache);

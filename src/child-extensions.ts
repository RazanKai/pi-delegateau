...
export interface ProbedTool {
  name: string;
  source?: { path?: string; source?: string; baseDir?: string };
}

function expandHome(value: string): string {
  if (value === "~" || value.startsWith("~/") || value.startsWith("~\\")) return os.homedir() + value.slice(1);
  return value;
}
...
function readManifest(root: string): PackageManifest {
  ...
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error(`Child extension package manifest is invalid at ${manifestPath}`);
  return parsed as PackageManifest;
}

function owningPackage(entry: string): { root: string; manifest: PackageManifest } {
  let directory = fs.statSync(entry).isDirectory() ? entry : path.dirname(entry);
  for (;;) {
    const manifestPath = path.join(directory, "package.json");
    if (fs.existsSync(manifestPath)) return { root: directory, manifest: readManifest(directory) };
    const parent = path.dirname(directory);
    if (parent === directory || path.basename(directory) === "node_modules") break;
    directory = parent;
  }
  throw new Error(`Child extension path is not owned by a package manifest: ${entry}`);
}

function assertExtensionManifest(root: string, manifest: PackageManifest): void {
...
function packageAliases(resource: ResolvedResource): string[] {
...
function rejectRecursive(root: string, manifest: PackageManifest): void {
...
async function resolveExplicit(
  selector: string,
  configured: ResolvedResource[],
  manager: DefaultPackageManager,
): Promise<string[]> {
  const expanded = expandHome(selector);
  if (!path.isAbsolute(expanded)) throw new Error(`Child extension paths must be absolute or start with ~: ${selector}`);
  const selectedPath = real(expanded);
  if (!fs.existsSync(selectedPath)) throw new Error(`Child extension path does not exist: ${selector}`);
  ...
  if (candidates.length === 0) throw new Error(`Child extension path is not declared by its package pi.extensions manifest: ${selector}`);
...
export async function resolveChildExtensions(
  selectors: string[],
  cwd: string,
  agentDir = getAgentDir(),
): Promise<ResolvedChildExtensions> {
  if (selectors.length === 0) return { paths: [], packageNames: [] };
  ...
  for (const selector of selectors) {
    ...
    if (!isPath) {
      ...
      if (roots.size > 1) throw new Error(`Child extension name "${selector}" is ambiguous across Pi packages`);
    }
    const matches = isPath
      ? await resolveExplicit(selector, configured, manager)
      : matchingResources.map((resource) => real(resource.path));
    if (matches.length === 0) throw new Error(`Child extension "${selector}" could not be resolved from Pi package settings`);

    for (const entry of matches) {
      if (!fs.existsSync(entry) || !fs.statSync(entry).isFile()) throw new Error(`Resolved child extension entry is missing: ${entry}`);
      const owner = owningPackage(entry);
      rejectRecursive(owner.root, owner.manifest);
      assertExtensionManifest(owner.root, owner.manifest);
      if (typeof owner.manifest.name === "string") packageNames.add(owner.manifest.name);
...
export function validateProbedChildTools(
  ...
  for (const name of requestedTools) {
    ...
    if (CHILD_TOOLS.has(name)) continue;
    const tool = byName.get(name);
    const sourcePath = tool?.source?.path ? real(tool.source.path) : undefined;
    if (!tool || tool.source?.source === "builtin" || !sourcePath || !allowedEntries.has(sourcePath)) invalid.push(name);
  }
  if (invalid.length > 0) throw new Error(`Child tools are not provided by approved built-ins or allowlisted extensions: ${invalid.join(", ")}`);

...
export class JevSelector implements ChoiceRuntime {
  ...
  async choose(input: JevChoiceInput): Promise<JevChoiceAnswer> {
    ...
    if (!answer || typeof answer.choice !== "string") throw new Error("Jev returned no model choice");
...
function parseIdentity(value: string) {
  ...
  if (slash <= 0 || slash === value.length - 1) throw new Error(`Jev returned malformed model identity: ${value}`);
  return { provider: value.slice(0, slash), id: value.slice(slash + 1) };
}

function isBoundedProbabilityMap(value: unknown, candidateIds: string[]): Record<string, number> | undefined {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return undefined;
  const allowed = new Set(candidateIds);
  const result: Record<string, number> = {};
  let any = false;
  for (const [key, raw] of Object.entries(value)) {
    if (!allowed.has(key)) continue; // keys outside the candidate set are dropped
    if (typeof raw !== "number" || !Number.isFinite(raw)) continue; // invalid values are dropped
    result[key] = raw;
    any = true;
  }

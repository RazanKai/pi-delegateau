...
const WEB_ACCESS_EXTENSIONS = ["pi-web-access", "donsetch"] as const;
export function detectWebAccessExtensions(tools: readonly unknown[]): string[] {
  return WEB_ACCESS_EXTENSIONS.filter((extension) => tools.some((tool) => {
    if (typeof tool !== "object" || tool === null) return false;
    const sourceInfo = (tool as { sourceInfo?: { path?: unknown; source?: unknown; baseDir?: unknown } }).sourceInfo;
...
const roles: Record<string, { instructions: string; tools: string[]; webAccess?: boolean; childExtensions?: string[] }> = {

...
function withCategory(
  base: { identity: ModelIdentity; reachable: false; latencyMs?: number; probedAt: string },
  errorCategory: ProbeResult["errorCategory"],
): ProbeResult {
...
export function probeCachePath(agentDir = getAgentDir()): string {
 ...
 */
export function categorizeProbeError(message: string): ProbeResult["errorCategory"] {
  const text = message.toLowerCase();
  if (text.includes("not supported when using codex") || text.includes("model is not supported")) return "unsupported-model";
  if (text.includes("free tier") || text.includes("can only be used from within")) return "unsupported-model";
  if (text.includes("401") || text.includes("auth") || text.includes("credential") || text.includes("api key")) return "auth";
  if (text.includes("quota") || text.includes("429") || text.includes("rate limit") || text.includes("usage limit")) return "quota";
  if (text.includes("timed out") || text.includes("timeout")) return "timeout";
  if (text.includes("enotfound") || text.includes("econnrefused") || text.includes("network") || text.includes("fetch failed")) return "network";
  return "unknown-error";
}

export function readProbeCache(filePath = probeCachePath()): ProbeCache | undefined {
  try {
    const raw = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (!raw || raw.version !== PROBE_CACHE_VERSION || !Array.isArray(raw.results)) return undefined;
    return raw as ProbeCache;
  } catch {
    return undefined;
  }
}

export function isProbeCacheFresh(cache: ProbeCache | undefined, now = Date.now()): boolean {
...
export function writeProbeCache(cache: ProbeCache, filePath = probeCachePath()): void {
...
/**
 * Whether one probe result is still usable. Freshness is per result, not per
 * cache file: `startBackgroundProbe` merges older entries into a newer cache,
 * so the top-level timestamp says when the file was last written, not when this
 * model was last probed. A missing, malformed or future-dated result timestamp
 * is never fresh, so such an entry cannot exclude its model.
 */
function isProbeResultFresh(result: ProbeResult, now: number): boolean {
  const at = Date.parse(result.probedAt);
  if (!Number.isFinite(at)) return false;
  // A timestamp after `now` is invalid (clock skew or a corrupt merge) and must
  // not become a fresh exclusion stretching into the future. Exactly 24h old
  // remains stale, as before.
 ...
 */
export function isKnownUnreachable(identity: ModelIdentity, cache: ProbeCache | undefined, now = Date.now()): boolean {
  if (!cache) return false;
  const found = cache.results.find((r) => r.identity.provider === identity.provider && r.identity.id === identity.id);
  if (!found || found.reachable) return false;
  return isProbeResultFresh(found, now);
}
 ...
 * dispatch fails.
 */
export function probeModel(identity: ModelIdentity, options: ProbeRunOptions): Promise<ProbeResult> {
 ...
 */
export async function runProbe(options: ProbeRunOptions): Promise<ProbeCache> {
  ...
  const cache: ProbeCache = { version: PROBE_CACHE_VERSION, probedAt: new Date().toISOString(), results };
  return cache;
}
...
export function startBackgroundProbe(
  options: ProbeRunOptions & { cachePath?: string; force?: boolean },
): { abort: () => void; done: Promise<ProbeCache | undefined> } {
  ...
  const done = (async () => {
    try {
      ...
      for (const r of existing?.results ?? []) merged.set(`${r.identity.provider}/${r.identity.id}`, r);
      for (const r of fresh.results) merged.set(`${r.identity.provider}/${r.identity.id}`, r);
      const cache: ProbeCache = { version: PROBE_CACHE_VERSION, probedAt: fresh.probedAt, results: [...merged.values()] };
...
export function probeWorkingDir(): string {

...
function identityFromEvidence(value: string | undefined): ModelIdentity | undefined {
 ...
 */
function appendBounded(current: string, next: string, limit: number): { text: string; truncated: boolean } {
  if (limit <= 0) return { text: "", truncated: current.length > 0 || next.length > 0 };
...
function mergeUsage(total: ChildResult["usage"] | undefined, event: ChildResult["usage"] | undefined): ChildResult["usage"] | undefined {
  if (!event) return total;
  const merged: NonNullable<ChildResult["usage"]> = { ...(total ?? {}) };
  const add = (key: keyof NonNullable<ChildResult["usage"]>, value: number | undefined) => {
    if (value === undefined) return;
    const prior = (merged[key] as number | undefined) ?? 0;
    (merged as Record<string, number>)[key] = prior + value;
  };
...
export class ChildRunner {
  ...
  async run(request: ChildRequest, onProgress?: (text: string) => void): Promise<ChildResult> {
    ...
    let servedModel: ModelIdentity | undefined;
    let sawAssistantCompletion = false;
    let sawAnyAssistant = false;
    let turns = 0;
    let providerError: string | undefined;
    let incompleteStopReason: string | undefined;
    let usage: ChildResult["usage"];
    ...
    try {
      ...
      const process = await this.spawner.spawn(spawnRequest, (event) => {
        if (event.type === "assistant") {
          ...
          if (event.errorMessage || stop === "error") providerError = event.errorMessage ?? "Child provider reported an error";
          ...
          if (diagnostics.length < 200) diagnostics.push(event.text.slice(-2_000));
        }
      });

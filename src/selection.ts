...
function findCandidate(request: DelegateRequest, identity: ModelIdentity) {
  return request.candidates.find((candidate) => sameModel(candidate.identity, identity));
}

function requireDefault(request: DelegateRequest): ModelIdentity {
  if (!request.defaultModel) throw new Error("No eligible models and no configured eligible default");
  if (!findCandidate(request, request.defaultModel)) throw new Error(`Configured default ${modelKey(request.defaultModel)} is not eligible`);
...
export async function selectModel(request: DelegateRequest, runtime: ChoiceRuntime): Promise<SelectionResult> {
  validateRequest(request);
  const started = performance.now();
  const pin = request.agent.model;
  if (pin) {
    if (!findCandidate(request, pin)) throw new Error(`Trusted pin ${modelKey(pin)} is not eligible`);
  ...
  if (request.candidates.length === 0) throw new Error("No eligible models");
  if (request.candidates.length === 1) return { identity: request.candidates[0]!.identity, source: "single-candidate" };
